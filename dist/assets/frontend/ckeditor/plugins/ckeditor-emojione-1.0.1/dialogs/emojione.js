...
CKEDITOR.dialog.add('emojioneDialog', function (editor)
{
    ...
    var buildHtml = function buildHtml(group)
    {
        var labelId = CKEDITOR.tools.getNextId() + '_smiley_emtions_label';
        var html = ['<div>' + '<span id="' + labelId + '" class="cke_voice_label">Test</span>', '<table role="listbox" aria-labelledby="' + labelId + '" style="width:100%;height:100%;border-collapse:separate;" cellspacing="2" cellpadding="2"', CKEDITOR.env.ie && CKEDITOR.env.quirks ? ' style="position:absolute;"' : '', '><tbody>'];
        ...
        for (var shortcode in emojione.emojioneList)
        {
            ...
            if (!emojione.emojioneList.hasOwnProperty(shortcode))
            {
                continue;
            }
        ...
        for (var shortcode in list)
        {
            ...
            if (!list.hasOwnProperty(shortcode))
            {
                continue;
            }
            ...
            html.push('<td class="cke_centered" style="vertical-align: middle;" role="presentation">' + '<a data-unicode="' + obj.unicode[0] + '" data-shortcode="' + shortcode + '" href="javascript:void(0)" role="option"', ' aria-posinset="' + (i + 1) + '"', ' aria-setsize=""', ' aria-labelledby=""', ' class="cke_hand" tabindex="-1" onkeydown="CKEDITOR.tools.callFunction( ', onKeydown, ', event, this );">', emojione.shortnameToUnicode(shortcode) + '</a>', '</td>');
    ...
    var emojis = function emojis(group)
    {
        return {
            type: 'html',
            id: 'emojiSelector',
    ...
    return {
        ...
        contents: [{
            id: 'tab-people',
            label: 'People',
            elements: [emojis('people')]
        }, {
            id: 'tab-nature',
            label: 'Nature',
            elements: [emojis('nature')]
        }, {
            id: 'tab-objects',
            label: 'Objects',
            elements: [emojis('objects')]
        }, {
            id: 'tab-places',
            label: 'Places',
            elements: [emojis('places')]
        }, {
            id: 'tab-symbols',
            label: 'Symbols',
            elements: [emojis('symbols')]
        }],
...
//# sourceMappingURL=emojione.js.map