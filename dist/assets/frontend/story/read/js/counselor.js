    ...
    $(function ()
    {
        ...
        function populateChat(data)
        {
            ...
            for (var _i = -1, _is = data.length; ++_i < _is;)
            {
                ...
                {
                    //right
                    container.append('' + '<div class="direct-chat-msg right">' + '<div class="direct-chat-info clearfix">' + '<span class="direct-chat-name pull-right">' + (chat['assign']['role'] === 'counselor' ? '<strong>[Counselor]</strong>' : "") + '&nbsp;&nbsp;<abbr title="' + chat['assign']['email'] + '">' + chat['assign']['username'] + '</abbr></span>' + '<span class="direct-chat-timestamp pull-left">' + chat['create_at'] + '</span>' + '</div>' + '<img class="direct-chat-img" src="' + chat['assign']['avatar'] + '" alt="Message User Image" width="96px">' + '<div class="direct-chat-text">' + chat['text'] + '</div>' + '</div>');
                } else
                {
                    container.append('' + '<div class="direct-chat-msg left">' + '<div class="direct-chat-info clearfix">' + '<span class="direct-chat-name pull-left"><abbr title="' + chat['assign']['email'] + '">' + chat['assign']['username'] + '</abbr>&nbsp;&nbsp;' + (chat['assign']['role'] === 'counselor' ? '<strong>[Counselor]</strong>' : "") + '</span>' + '<span class="direct-chat-timestamp pull-right">' + chat['create_at'] + '</span>' + '</div>' + '<img class="direct-chat-img" src="' + chat['assign']['avatar'] + '" alt="Message User Image" width="96px">' + '<div class="direct-chat-text">' + chat['text'] + '</div>' + '</div>');
                }
            }
        }
    });
